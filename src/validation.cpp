...
#if defined(NDEBUG)
# error "R3VCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "R3VCoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
    ...
    if(chainActive.Tip()->nHeight > Params().LastProofOfWorkHeight() && tx.nVersion <= POW_TX_VERSION) {
        return state.DoS(100, false, REJECT_NONSTANDARD, "tx-old-pos-version", true);
    }
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s R3V additional fees, %d delta bytes\n",
    ...
    if (fTxIndex) {
        ...
        if (pblocktree->ReadTxIndex(hash, postx)) {
            ...
            try {
                ...
                return error("%s: header Deserialize or I/O error - %s", __func__, e.what());
            }
            try {
                fseek(file.Get(), postx.nTxOffset, SEEK_CUR);
                file >> txOut;
            } catch (const std::exception& e) {
                return error("%s: txOut Deserialize or I/O error - %s", __func__, e.what());
            }
    ...
    bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
    {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if ((unsigned int)coins->nTime > tx.nTime)
                return state.DoS(100, false, REJECT_INVALID, strprintf("transaction timestamp %u earlier than input transaction %s", tx.nTime, coins->nTime));
...
bool DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    int blocksize = block.IsProofOfStake() ? 1 : 0;
    for (int i = block.vtx.size() - 1; i >= blocksize; i--) {
    ...
    if (!fBare) {
        ...
        for (const auto& it : block.vtx) {
            ...
            if (tx.IsCoinStake())
            {
                ...
                GetMainSignals().UpdatedTransaction(hashTx);
            }
            else
            {
...
bool AddToBlockIndex(const CBlockHeader& block, CBlockIndex** pindex, const uint256 &hashProof)
{
    CBlockIndex *&pindexNew = *pindex;
...
bool AddToBlockIndex(const CBlock& block, CBlockIndex** pindex, const uint256 &hashProof)
{
    CBlockIndex *&pindexNew = *pindex;
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    ...
    if (block.GetBlockTime() <= CHECK_POW_FROM_NTIME && fCheckPOW && !CheckProofOfWork(block.GetPoWHash(), block.nBits, consensusParams))
        return state.DoS(50, false, REJECT_INVALID, "high-hash", false, "proof of work failed");

    return true;
}

bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig, bool fCheckFlood)
{
    ...
    bool PosDuplicate = false;
    if (fCheckFlood && block.IsProofOfStake() && setStakeSeen.count(block.GetProofOfStake())) {
        // Check is the stake tx is belong to this block
        BlockMap::iterator it = mapBlockIndex.find(hash);
        if (it != mapBlockIndex.end()) {
            if (it->second->prevoutStake == block.GetProofOfStake().first && it->second->nStakeTime == block.GetProofOfStake().second) {
                //Match block index, prevoutStake and nStakeTime
            } else {
                PosDuplicate = true;
            }
        } else {
    ...
    if (PosDuplicate) {
        return error("ProcessBlock() : duplicate proof-of-stake (%s, %d) for block %s", 
                        block.GetProofOfStake().first.ToString().c_str(), 
                        block.GetProofOfStake().second, 
                        hash.ToString().c_str());
    }
    ...
    if (block.IsProofOfStake())
    {
        ...
        if (block.vtx[0]->vout.size() != 1) {
            return state.DoS(100, false, REJECT_INVALID, "bad-pos-cb", false, "coinbase output not empty for proof-of-stake block 1");
        }
        if (block.vtx[0]->vout.size() != 1 || !block.vtx[0]->vout[0].IsEmpty()) {
            return state.DoS(100, false, REJECT_INVALID, "bad-pos-cb", false, "coinbase output not empty for proof-of-stake block 2");
        }

        // Second transaction must be coinstake, the rest must not be
        if (block.vtx.empty() || !block.vtx[1]->IsCoinStake()) {
            return state.DoS(100, false, REJECT_INVALID, "bad-tx-cs", false, "second tx is not coinstake");
        }
        ...
        if (!CheckCoinStakeTimestamp(block.GetBlockTime(), (int64_t)block.vtx[1]->nTime)) {
            return state.DoS(50, false, REJECT_INVALID, "bad-cs-timestamp", false, strprintf("coinstake timestamp violation nTimeBlock=%s nTimeTx=%u", block.GetBlockTime(), block.vtx[1]->nTime));
        }

        // Check proof-of-stake block signature
        if (fCheckSig && !CheckBlockSignature(block)) {
            return state.DoS(100, false, REJECT_INVALID, "bad-pos-sign", false, "bad proof-of-stake block signature");
        }
    }
...
static int GetWitnessCommitmentIndex(const CBlock& block)
{
    ...
    if (!block.vtx.empty()) {
        if (block.IsProofOfStake()) {
            commitpos = 0; //no withness for pos
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (block.IsProofOfWork() && nHeight > Params().LastProofOfWorkHeight())
        return state.DoS(100, false, REJECT_INVALID, "pow-end", false, strprintf("reject proof-of-work at height %d", nHeight));
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (pindex == NULL) {
        pindex = new CBlockIndex(block);
        if (!AddToBlockIndex(block, &pindex, hashProof)) {
            return error("AcceptBlockHeader() : AddToBlockIndex failed");
        }
    }
...
static bool AcceptBlockHeader(const CBlock& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (block.IsProofOfStake())
        {
            ...
            {
                LogPrintf("WARNING: AcceptBlockHeader(): check proof-of-stake failed for block %s\n", hash.ToString().c_str());
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // R3VCoin: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (nCheckLevel >= 1 && !CheckBlock(block, state, chainparams.GetConsensus(), true, true, true, false))
...
bool InitBlockIndex(const CChainParams& chainparams)
{
    ...
    if (!fReindex) {
        try {
            ...
            if (!AddToBlockIndex(block, &pindex, chainparams.GetConsensus().hashGenesisBlock)) {
...
bool VerifySignature(const CTransactionRef& txFrom, const CTransactionRef& txTo, unsigned int nIn)
{
    ...
    const CTransaction& ctxTo = *txTo;
    //const CTransaction& ctxFrom = *txFrom;
    
    return VerifyScript(txin.scriptSig, txout.scriptPubKey, witness, (SCRIPT_VERIFY_P2SH), 
        TransactionSignatureChecker(&ctxTo, nIn, txFrom->vout[txTo->vin[nIn].prevout.n].nValue), NULL);
}
