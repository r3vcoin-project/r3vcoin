...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFlushOnClose)
{
    ...
    //// debug print
    LogPrintf("AddToWallet %s(%u)  %s%s\n", wtxIn.GetHash().ToString(), wtxIn.nIndex, (fInsertedNew ? "new" : ""), (fUpdated ? "update" : ""));
...
void CWallet::AvailableCoinsMinConf(vector<COutput>& vCoins, int nConf) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (!IsFinalTx(*pcoin->tx, chainActive.Height() + 1, GetAdjustedTime()))
                continue;
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                if (!(IsSpent(wtxid, i)) && IsMine(pcoin->tx->vout[i]) &&
...
bool CWallet::GetStakeWeight(uint64_t& nAverageWeight, uint64_t& nTotalWeight)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        {
            LOCK2(cs_main, cs_wallet);
            if (!GetTransaction(pcoin.first->GetHash(), ctx, Params().GetConsensus(), hashBlock, true))
                continue;
            if (!mapBlockIndex.count(hashBlock))
                continue;
        }
        ...
        unsigned int nTimeTx = ctx->nTime ? ctx->nTime : mapBlockIndex[hashBlock]->nTime;
        ...
        if (nTimeWeight > 0)
        {
            nTotalWeight += ArithToUint256(bnCoinDayWeight).GetUint64(0);
            nWeightCount++;
        }
    }
...
bool CWallet::CreateCoinStake(unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CMutableTransaction& txNew, CKey& key)
{
    ...
    vector<const CWalletTx*> vwtxPrev;
    vector<CTxOut> voutPrev; //Temporary storage to be manipulated and stored in txNew later
    voutPrev.clear();
    voutPrev.push_back(CTxOut(0, scriptEmpty));
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        if (block.GetBlockTime() + Params().StakeMinAge() > txNew.nTime)
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound && pindexPrev == pindexBest; n++)
        {
            ...
            if (CheckStakeKernelHash(nBits, block, prevoutStake.n, pcoin.first->tx, prevoutStake, txNew.nTime, hashProofOfStake, targetProofOfStake, fDebug))
            {
                // Found a kernel
                ...
                txNew.vin.push_back(CTxIn(pcoin.first->GetHash(), pcoin.second, CScript(), std::numeric_limits<unsigned int>::max() - (fWalletRbf ? 2 : 1)));
                nCredit += pcoin.first->tx->vout[pcoin.second].nValue;
                vwtxPrev.push_back(pcoin.first);
                voutPrev.push_back(CTxOut(0, scriptPubKeyOut));
                txNew.vout.push_back(CTxOut(0, scriptPubKeyOut));

                if (GetCoinAgeWeight(block.GetBlockTime(), (int64_t)txNew.nTime) < nStakeSplitAge && nCredit >= nStakeCombineThreshold) {
                    voutPrev.push_back(CTxOut(0, scriptPubKeyOut)); //split stake
                    txNew.vout.push_back(CTxOut(0, scriptPubKeyOut)); 
                }
                if (fDebug && GetBoolArg("-printcoinstake", false))
    ...
    // Calculate coin age reward
    {
        ...
        if (nReward <= 0 && COIN_YEAR_REWARD > 0) {
            return false;
        }
    ...
    // Set output amount
    if (voutPrev.size() == 3) {
        voutPrev[1].nValue = (nCredit / 2 / CENT) * CENT;
        voutPrev[2].nValue = nCredit - voutPrev[1].nValue;
    } else {
        voutPrev[1].nValue = nCredit;
    }

    //Apply mutable vout to txNew
    txNew.vout.clear();
    for (unsigned int x = 0; x < voutPrev.size(); x++) {
        txNew.vout.push_back(CTxOut(voutPrev[x].nValue, voutPrev[x].scriptPubKey)); 
    }

    // Sign
    CTransaction txNewConst(txNew);
    int nIn = 0;
    for (const auto& coin : vwtxPrev)
    {
        const CTxIn& txin = txNewConst.vin[nIn];

        const CScript& scriptPubKey = coin->tx->vout[txin.prevout.n].scriptPubKey;
        SignatureData sigdata;

        if (!ProduceSignature(TransactionSignatureCreator(this, 
            &txNewConst, 
            nIn, 
            coin->tx->vout[txin.prevout.n].nValue, 
    ...
    if (nBytes >= MAX_BLOCK_BASE_SIZE/5) {
    ...
    if (nSearchTime > nLastCoinStakeSearchTime)
    {
        ...
        if (CreateCoinStake(pblock->nBits, nSearchTime-nLastCoinStakeSearchTime, nFees, txCoinStake, key))
        {
            ...
            if (txCoinStake.nTime >= max(pindexBest->GetMedianTimePast()+1, PastDrift(pindexBest->GetBlockTime())))
            {
                ...
                CMutableTransaction pblock_tx(*pblock->vtx[0]);
                pblock_tx.nTime = txCoinStake.nTime;
                ...
                pblock->nTime = max(pblock->nTime, txCoinStake.nTime);*/
                //txCoinStake.nTime = pblock->nTime;

                // we have to make sure that we have no future timestamps in
                //    our transactions set
                /*for (vector<CTransactionRef>::iterator it = pblock->vtx.begin(); it != pblock->vtx.end();)
                {
                    if (it == pblock->vtx.begin()) {
                        if (pblock_tx.nTime > pblock->nTime) {
                            it = pblock->vtx.erase(it); 
                        } else { 
                            ++it; 
                        }
                    } else {
                        const CTransactionRef& ctx = *it;
                        if (ctx->nTime > pblock->nTime) { 
                            it = pblock->vtx.erase(it); 
                        } else { 
                            ++it; 
                        }
                ...
                int vtx_size = pblock->vtx.size();
                if (vtx_size > 1) {
                    //Switch coinstake to 2nd position
                    std::vector<CTransactionRef> vtx;
                    vtx.push_back(MakeTransactionRef(std::move(pblock_tx)));
                    vtx.push_back(MakeTransactionRef(txCoinStake));
                    for (int i = 1; i < vtx_size; i++) {
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-stakenotify=<cmd>", _("Execute command when a coinstake transaction is created (%s in cmd is replaced by TxID)"));
...
bool CWallet::ParameterInteraction()
{
    ...
    if (IsArgSet("-reservebalance")) // PoSV: reserve balance amount
    {
        CAmount nReserveBalance = 0;
        if (!ParseMoney(GetArg("-reservebalance", ""), nReserveBalance))
            return InitError(strprintf(_("Invalid amount for -reservebalance=<amount>: '%s'"), GetArg("-paytxfee", "")));
    }
