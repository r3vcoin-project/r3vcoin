...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    nLockTimeCutoff = (STANDARD_LOCKTIME_VERIFY_FLAGS & LOCKTIME_MEDIAN_TIME_PAST)
                       ? nMedianTimePast
                       : pblock->GetBlockTime();
    nPoSTimeCutoff = pblock->GetBlockTime();
    ...
    if (!fProofOfStake)
    {
        ...
        coinbaseTx.nVersion = POW_TX_VERSION;
    ...
    else
    {
        ...
        coinbaseTx.vin[0].scriptSig = (CScript() << chainActive.Tip()->nHeight+1) + COINBASE_FLAGS;
...
bool BlockAssembler::TestPackageTransactions(const CTxMemPool::setEntries& package)
{
    ...
    BOOST_FOREACH (const CTxMemPool::txiter it, package) {
        ...
        // Transaction time must be later than block time
        if (it->GetSharedTx()->nTime > nPoSTimeCutoff) {
            return false;
        }
...
bool BlockAssembler::TestForBlock(CTxMemPool::txiter iter)
{
    ...
    // Transaction time must be later than block time
    if (iter->GetSharedTx()->nTime > nPoSTimeCutoff) {
        return false;
    }
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlockWithKey(CReserveKey& reservekey)
{
    ...
    return CreateNewBlock(GetScriptForRawPubKey(pubkey));
}
...
bool CheckStake(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    LogPrintf("R3VCoinStaker: proof-of-stake found  \n  hash: %s\n  stake: %s\n  target: %s\n", hash.GetHex(), hashStake.GetHex(), hashTarget.GetHex());
    const CTransaction& cvtx = *pblock->vtx[1];
    LogPrintf("minted %s\n", FormatMoney(cvtx.GetValueOut()));

    // Found a solution
    {
        LOCK(cs_main);
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("R3VCoinStaker : minted block is stale");
        ...
        if (!ProcessNewBlock(Params(), shared_pblock, true, NULL))
            return error("R3VCoinStaker : ProcessBlock, block not accepted");
    }

    return true;
}

void CoinStaker(CWallet *pwallet)
{
    LogPrintf("CoinStaker started\n");
    ...
    try { while (true) {
        if (chain != CBaseChainParams::REGTEST) {
            ...
            while (!g_connman || g_connman->GetNodeCount(CConnman::CONNECTIONS_OUT) == 0)
            {
                // Busy-wait for the network to come online.
                LogPrintf("CoinStaker : Waiting for network online.\n");
        ...
        while (IsInitialBlockDownload()) 
        {
            ...
            LogPrintf("CoinStaker : Waiting... Blockchain Downloading.\n");
            MilliSleep(60000);
        }

        while (pwallet->IsLocked())
        {
            LogPrintf("CoinStaker : Wallet is locked.\n");
            nLastCoinStakeSearchInterval = 0;
            MilliSleep(1000);
        }
        ...
        {
            LogPrintf("CoinStaker : Chaintip < Last POW, %u < %u.\n", chainActive.Tip()->nHeight + 1, Params().LastProofOfWorkHeight());
            MilliSleep(60000);
        }
        ...
        std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(Params()).CreateNewBlockWithKey(reservekey));
        if (!pblocktemplate.get())
            return;
        ...
        {
            if (fDebug) {
                LogPrintf("CoinStaker : Failed to sign the new block.\n");
            }
            MilliSleep(1000);
        }
    } }
    ...
    {
        LogPrintf("CoinStaker terminated\n");
        throw;
    }
}
...
void GenerateCoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    minerThreads->create_thread(boost::bind(&CoinStaker, pwallet));
}
