...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::GETHEADERS)
    {
        ...
        {
            const CBlockHeader& header = vHeaders[i];
            LogPrintf("GETHEADERS() - n: %u, nVersion: %u, hashPrevBlock: %s, hashMerkleRoot: %s, nTime: %u, nBits: %u, nNonce: %u\n", 
                i, header.nVersion, header.hashPrevBlock.ToString(), header.hashMerkleRoot.ToString(), header.nTime, header.nBits, header.nNonce);
        }*/
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        for (unsigned int n = 0; n < nCount; n++) {
            ...
            //ReadCompactSize(vRecv); // ignore vchBlockSig; assume it is empty.

            /*const CBlockHeader& header = headers[n];
            LogPrintf("HEADERS() - n: %u, nVersion: %u, hashPrevBlock: %s, hashMerkleRoot: %s, nTime: %u, nBits: %u, nNonce: %u\n", 
                n, header.nVersion, header.hashPrevBlock.ToString(), header.hashMerkleRoot.ToString(), header.nTime, header.nBits, header.nNonce);*/
        }
        ...
        for (const CBlockHeader& header : headers) {
            if (!hashLastBlock.IsNull() && header.hashPrevBlock != hashLastBlock) {
                Misbehaving(pfrom->GetId(), 20);
                //LogPrintf("HEADERS() - hashPrevBlock: %s, hashLastBlock: %s\n", header.hashPrevBlock.ToString(), hashLastBlock.ToString());
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (pblock->IsProofOfWork()) {
            CBlock& block = *pblock;
            CMutableTransaction vtx(*block.vtx[0]);
            vtx.nTime = 0;

            block.vtx[0] = MakeTransactionRef(std::move(vtx));
