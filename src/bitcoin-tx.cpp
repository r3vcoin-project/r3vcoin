...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s r3vcoin-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  r3vcoin-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded r3vcoin transaction") + "\n" +
              "  r3vcoin-tx [options] -create [commands]   " + _("Create hex-encoded r3vcoin transaction") + "\n" +
              "\n";
...
{
    int64_t newNtime = atoi64(cmdVal);
    if (newNtime < 0LL || newNtime > 0xffffffffLL)
        throw std::runtime_error("Invalid TX ntime requested");
