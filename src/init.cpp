...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (showDebug)
    {
        strUsage += HelpMessageOpt("-printpriority", strprintf("Log transaction priority and fee per kB when staking blocks (default: %u)", DEFAULT_PRINTPRIORITY));
    }
...
#ifdef ENABLE_WALLET
    if (pwalletMain) {
        GenerateCoins(GetBoolArg("-staking", true), pwalletMain, GetArg("-genproclimit", 1));
