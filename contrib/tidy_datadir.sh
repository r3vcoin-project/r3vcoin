...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete R3VCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no R3VCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old R3VCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected R3VCoin 0.7 datadir."
    ;;
  3)
    echo "Detected R3VCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected R3VCoin 0.8 datadir."
    ;;
esac
