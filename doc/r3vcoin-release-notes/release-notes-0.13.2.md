R3V Coin Core version 0.13.2 is now available from:
...
Previous versions of R3V Coin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of R3V Coin Core was to raise the default minimum
relay fee.

R3V Coin Core 0.13.2 will have a strict maximum size on the mempool. The
...
R3V Coin Core 0.13.2 also introduces new default policy limits on the length and
...
R3V Coin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. R3V Coin Core will relay transactions with
...
entirely in R3V Coin Core 0.13, and it is at this time undecided whether the
...
R3V Coin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
R3V Coin Core automatically creates a hidden service to listen on, without
manual configuration. R3V Coin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if R3V Coin Core is listening, and
...
Core to use floating fees. By default, R3V Coin Core will use floating
fees.
...
At all times, R3V Coin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) R3V.
Furthermore, R3V Coin Core will never create transactions paying less than
...
caching. A sample config for apache2 could look like:
    ...
    <Location /r3vcoinrpc>
        ProxyPass http://127.0.0.1:13844/
        ProxyPassReverse http://127.0.0.1:13844/
...
Various code modernizations have been done. The R3V Coin Core code base has
...
HD wallets are incompatible with older versions of R3V Coin Core.
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  note that R3V Coin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  R3VCoin users to individually opt-in to almost any change in the R3VCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and R3V Coin Core contributors that may
...
client to R3V Coin Core 0.13.2 to signal for segwit support. When a
...
Since R3V Coin Core 0.10.0, nodes have defaulted to only relaying and
