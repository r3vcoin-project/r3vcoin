R3VCoin Core version 0.13.2 is now available from:

  <https://download.r3vcoin.org/r3vcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/r3vcoin-project/r3vcoin/issues>
...
updates will be released anymore. Without security updates, using a r3vcoin
...
ECDSA signatures inside R3VCoin transactions now use validation using
...
Previous versions of R3VCoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of R3VCoin Core was to raise the default minimum
relay fee.

R3VCoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
R3VCoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running r3vcoind without having to do any manual configuration.
...
R3VCoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. R3VCoin Core will relay transactions with
...
entirely in R3VCoin Core 0.13, and it is at this time undecided whether the
...
R3VCoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
R3VCoin Core automatically creates a hidden service to listen on, without
manual configuration. R3VCoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if R3VCoin Core is listening, and
...
R3VCoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals R3VCoin
Core to use floating fees. By default, R3VCoin Core will use floating
fees.
...
(default: `0.0002` R3V/kB).

At all times, R3VCoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) R3V.
Furthermore, R3VCoin Core will never create transactions paying less than
...
`r3vcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 214844 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 214844 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /r3vcoinrpc>
        ProxyPass http://127.0.0.1:14844/
        ProxyPassReverse http://127.0.0.1:14844/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass r3vcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< r3vcoinrpc:<password>
    </Location>
...
Restarting r3vcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in r3vcoin.conf
...
For example:
    ...
    $ src/r3vcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The R3VCoin Core code base has
...
- `r3vcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `r3vcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the R3VCoin scripting system that in combination with
...
R3VCoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of R3VCoin Core.
...
The two phases are distinct in the R3VCoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming R3VCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for R3VCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each R3VCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of r3vcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of r3vcoins being spent is exactly
...
- **Increased security for multisig:** R3VCoin addresses (both P2PKH addresses
  ...
  believed to be provided by R3VCoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of R3VCoin's current ledger (called the UTXO set),
  ...
  note that R3VCoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  R3VCoin users to individually opt-in to almost any change in the R3VCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and R3VCoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the R3VCoin
client to R3VCoin Core 0.13.2 to signal for segwit support. When a
...
Since R3VCoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `r3vcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `r3vcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
