...
*Setup instructions for a Gitian build of R3V Coin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the R3VCoin
...
- [Building R3V Coin Core](#building-r3vcoin-core)
...
Clone the git repositories for r3vcoin and Gitian.
...
in the r3vcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building R3V Coin Core
----------------

To build R3V Coin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the r3vcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/r3vcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/r3vcoin-project/r3vcoin
...
For example:
```bash
URL=https://github.com/thrasher-/r3vcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit r3vcoin=${COMMIT} --url r3vcoin=${URL} ../r3vcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit r3vcoin=${COMMIT} --url r3vcoin=${URL} ../r3vcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit r3vcoin=${COMMIT} --url r3vcoin=${URL} ../r3vcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the r3vcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../r3vcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/r3vcoin
SIGPATH=/some/root/path/r3vcoin-detached-sigs

./bin/gbuild --url r3vcoin=${BTCPATH},signature=${SIGPATH} ../r3vcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/r3vcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/r3vcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/r3vcoin-osx-build.assert
```
...
[r3vcoin-project/gitian.sigs.R3V](https://github.com/r3vcoin-project/gitian.sigs.R3V/) repository, or if that's not possible create a pull
